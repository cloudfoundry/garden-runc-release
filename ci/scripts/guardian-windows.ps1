...
if ((Get-Command "go.exe" -ErrorAction SilentlyContinue) -eq $null) {
  ...
  Invoke-WebRequest https://storage.googleapis.com/golang/go1.9.windows-amd64.msi -OutFile go.msi
...
ginkgo -r -p -race -keepGoing -failOnPending -skipPackage "dadoo,kawasaki,locksmith,socket2me" -focus "Runtime Plugin" gqt
Exit $LastExitCode
